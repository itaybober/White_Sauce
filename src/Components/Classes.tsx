...
class Mission {
   ...
   *
   */
  public async survivalPointSystem(
    player: Player,
    time: number,
    succeed = true,
  ) {
  ...
  static async addMissionToFirestore(mission: Mission): Promise<void> {
    ...
    try {
      mission._tags.map(async (tag) => {
        await setDoc(
          doc(db, "Missions", tag, tag, mission._title),
          missionData,
        );
      });
  ...
  public static getMissionObject(data: {
    title: string | undefined;
    description: string | undefined;
    tags: never[] | undefined;
    type: string | undefined;
    extras: never[] | undefined;
    minNumOfPlayers: number | undefined;
    maxNumOfPlayers: number | undefined;
  }) {
    return new Mission(
      data.title,
      data.description,
      ...
      data.type,
      data.extras,
      data.minNumOfPlayers,
      data.maxNumOfPlayers,
    );
  }
  //
...
class Player {
  ...
  constructor(
    UID: string = "null",
    name: string | null = "",
    gameRef: DocumentReference = doc(db, "Games", "0000"),
  ) {
    if (name === null) {
      this._name = "";
    } else {
  ...
  public async getSecretMissionFromDatabase() {
    ...
    const ourMission =
      ourMissions.docs[getRandomNumber(ourMissions.docs.length)];
...
class Game {
  ...
  public async assignsAvatar() {
    let avatarRef_list = [
      "https://firebasestorage.googleapis.com/v0/b/white-sauce.appspot.com/o/avatars%2Favatar.png?alt=media&token=cd3b92cf-c590-45a3-90a8-99003210bd7d",
