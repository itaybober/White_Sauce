...
export default function Punishment({ curPlayer, curGame }) {
  ...
  return (
    <Container className={"punishment_page_component"} sx={{ p: 2 }}>
          ...
          <div>
            ...
            >
              <Typography variant="h5" color={"primary"}>
                {" "}
                <b> {mission_object._title}</b>
              </Typography>
              <Typography variant={"h6"}>
                <br />
                <b>You lost, sucks to suck. </b>
                <br />
      ...
      {!isPictureUploaded && timeElapsed > 10 && (
        <Typography variant={"h5"}>
          {" "}
          <br></br>Loser - take a Loser photo{" "}
        </Typography>
      )}
      {isPictureUploaded && (
        <Typography variant={"h5"}>
      ...
      {timeElapsed > 10 && (
        <CameraComponent
          buttonText="Your disgraceful Picture"
          onPictureUpload={handlePictureUpload}
          curGameNum={curGame._id}
        />
      )}
      {!isPictureUploaded && (
        <Button
