...
function EndingPage({ curPlayer, curGame }: EndingPageProps) {
  ...
  return (
    <Container className={"Ending_page_component"} sx={{ p: 2 }}>
          ...
          {winner.split("").map((letter, index) => (
            <span
              ...
              style={{
                ...
                animation: `${
                  winner && winner.length ? "wave" : ""
                } 1s ease-in-out infinite ${index * 0.1}s`,
              }}
            >
              <b>{letter}</b>
            </span>
      ...
      <ImageList sx={{ width: 350 }} cols={2} variant="masonry" gap={10}>
      ...
      <Typography className={"journey-txt"} variant="h5">
        {" "}
        <br></br>
        <br />
        <i>
          {" "}
          "A wonderful journey begins with <b>one step" </b>
          And you just did it!
        </i>
      </Typography>
